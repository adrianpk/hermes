...
type User struct {
	...
	TeamPermissions map[uuid.UUID][]Permission `json:"teamPermissions,omitempty"`
...
func NewUser(username, name string) User {
	u := User{
		...
		TeamPermissions: make(map[uuid.UUID][]Permission),
		RoleIDs:         []uuid.UUID{},
